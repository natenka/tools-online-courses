...
ANSWERS_REPO = "https://github.com/natenka/pyneng-answers-en"
ANSWERS_DIR = "pyneng-answers-en"
TEACHER_GITHUB_USER = "pyneng"
TASK_DIRS = [
    "04_data_structures",
    "05_basic_scripts",
...
def copy_answers(passed_tasks):
    ...
    if os.path.exists(ANSWERS_DIR):
        shutil.rmtree(ANSWERS_DIR, onerror=remove_readonly)
    returncode, stderr = call_command(
        f"git clone --depth=1 {ANSWERS_REPO}",
        verbose=False,
        return_stderr=True,
    )
    if returncode == 0:
        os.chdir(f"{ANSWERS_DIR}/answers/{current_chapter_name}")
